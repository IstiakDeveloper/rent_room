    ...
    @if ($user->hasRole('User'))
        ...
        {{-- <div class="col-12 col-md-6 col-lg-6">
                <div class="card shadow-sm p-4">
                    ...
                    <form wire:submit.prevent="saveUserDetails" class="bg-light p-4 rounded shadow-sm">
                        ...
                        @if ($userDetail['package_id'])
                            <button type="button" class="btn btn-danger btn-sm ml-2" wire:click="removePackage"
                                title="Remove Package" @if (!$isDetailEditing) disabled @endif>
                                <i class="fas fa-trash-alt"></i> Remove Package
                            </button>
                        @endif

                        <div class="form-group mb-3">
                            <label for="occupied_address_details" class="form-label">
                                @if ($userDetail['stay_status'] === 'staying')
                                    Occupy Address
                                @else
                                    Inquire Address
                            ...
                            <input type="text" id="occupied_address_details" class="form-control"
                                placeholder="Address" wire:model="userDetail.occupied_address"
                                value="{{ old('userDetail.occupied_address', $userDetail['occupied_address']) }}"
                                @if (!$isDetailEditing) disabled @endif>
                        </div>

                        @if ($userDetail['stay_status'] === 'want_to')
                            <div class="form-group mb-3">
                                <label for="package_details" class="form-label">Package</label>
                                <input type="text" id="package_details" class="form-control" placeholder="Package"
                                    wire:model="userDetail.package"
                                    value="{{ old('userDetail.package', $userDetail['package']) }}"
                                    @if (!$isDetailEditing) disabled @endif>
                            </div>
                        @endif
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" id="booking_type" class="form-control"
                                placeholder="Entire Property / Room Number" wire:model="userDetail.booking_type"
                                value="{{ old('userDetail.booking_type', $userDetail['booking_type']) }}"
                                @if (!$isDetailEditing) disabled @endif>
                        </div>

                        <div class="form-group mb-3">
                            <label for="entry_date_details" class="form-label">Entry Date</label>
                            <input type="date" id="entry_date_details" class="form-control"
                                placeholder="Entry Date" wire:model="userDetail.entry_date"
                                value="{{ old('userDetail.entry_date', $userDetail['entry_date']) }}"
                                @if (!$isDetailEditing) disabled @endif>
                        </div>

                        <div class="form-group mb-3">
                            <label for="duration_type" class="form-label">Payment Frequency</label>
                            <select id="duration_type" class="form-control"
                                wire:model.defer="userDetail.duration_type"
                                @if (!$isDetailEditing) disabled @endif>
                                <option value="">Select Payment Frequency</option>
                                <option value="Weekly">Weekly</option>
                                <option value="Monthly">Monthly</option>
                            </select>
                            @error('userDetail.duration_type')
                                <div class="text-danger mt-1">{{ $message }}</div>
                            @enderror
                        </div>

                        <div class="form-group mb-3">
                            <label for="security_amount_details" class="form-label">
                                @if ($userDetail['stay_status'] === 'staying')
                                    Security Amount
                                @else
                                    Ability Amount
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" id="security_amount_details" step="0.01"
                                    class="form-control" placeholder="Amount" wire:model="userDetail.security_amount"
                                    value="{{ old('userDetail.security_amount', $userDetail['security_amount']) }}"
                                    @if (!$isDetailEditing) disabled @endif>
                            </div>
                        </div>

                        <div class="form-group mb-3">
                            <label for="security_payment_status" class="form-label">Security Payment Status</label>
                            <select id="security_payment_status" class="form-control"
                                wire:model.defer="userDetail.security_payment_status"
                                @if (!$isDetailEditing) disabled @endif>
                        ...
                        <div class="form-group mb-3">
                            <label for="package_price_details" class="form-label">
                                @if ($userDetail['stay_status'] === 'staying')
                                    Package Price
                                @else
                                    Budget Price
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" id="package_price_details" class="form-control"
                                    placeholder="Price" wire:model="userDetail.package_price"
                                    value="{{ old('userDetail.package_price', $userDetail['package_price']) }}"
                                    @if (!$isDetailEditing) disabled @endif>
                            </div>
                        </div>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select id="payment_status" class="form-control"
                                wire:model.defer="userDetail.payment_status"
                                @if (!$isDetailEditing) disabled @endif>
                                <option value="Pending">Pending</option>
                                <option value="Paid">Paid</option>
                            </select>
                        ...
                        @if ($userDetail['payment_status'] === 'Pending' || (isset($userDetail['security_payment_status']) && $userDetail['security_payment_status'] === 'Pending'))
                            <button type="button" class="btn btn-warning mt-3" wire:click="generatePaymentLink">
                        ...
                        <div class="d-flex justify-content-between mt-4">
                            ...
                            <button type="button" class="btn btn-outline-primary position-relative"
                                ...
                                <span wire:loading wire:target="generateInvoice"
                                    class="spinner-border spinner-border-sm ms-2 position-absolute top-50 end-0 translate-middle-y"
                                    role="status">
                ...
                <div class="card shadow-sm p-4 mt-4">
                    <div class="d-flex justify-content-between">
                        ...
                        <button type="button" class="btn btn-sm btn-secondary" wire:click="enablePaymentEditing"
                            @if ($isPaymentEditing) disabled @endif>
                            Edit Payment
                        </button>
                    </div>

                    <form wire:submit.prevent="{{ $editingPaymentId ? 'updatePayment' : 'savePayment' }}">
                        <div class="form-group mt-2">
                            <label for="package_price" class="form-label">Amount (£)</label>
                            <!-- Display the package price as read-only -->
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text">£</span>
                                </div>
                                <input type="text" id="package_price" class="form-control"
                                    wire:model="userDetail.package_price" disabled>
                            </div>
                        ...
                        <div class="form-group mt-2">
                            ...
                            <input type="date" id="payment_date" class="form-control"
                                wire:model="paymentDetail.payment_date"
                                @if (!$isPaymentEditing) disabled @endif>
                            @error('paymentDetail.payment_date')
                        ...
                        <div class="form-group mt-2">
                            ...
                            <textarea id="payment_details" class="form-control" wire:model="paymentDetail.details" placeholder="Payment details"
                                @if (!$isPaymentEditing) disabled @endif></textarea>
                            @error('paymentDetail.details')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mt-2">
                            <label for="payment_status" class="form-label">Payment Status</label>
                            <select id="payment_status" class="form-control"
                                wire:model.defer="paymentDetail.payment_status"
                                @if (!$isPaymentEditing) disabled @endif>
                                <option value="Pending">Pending</option>
                                <option value="Paid">Paid</option>
                            </select>
                        ...
                        <!-- Change button text dynamically for Create or Update -->
                    ...
                    <!-- List of Payments -->
                    <h5 class="mt-4">Previous Payments</h5>
